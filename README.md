...
> 무르지는 않는다 — 판정은 사람이 한다.
>
> **두 검사는 상보적이다.** 게이트 6개를 돌린 실측에서 정직하게 다시 돈 재확인의 비율은
> 최저 0.69였고 사후 감지의 오탐은 0이었다. 반대로 사전 경고는 그중 둘에 오탐을 냈다 —
> 그 게이트들은 빌드 스크립트에서 이미 캐시를 꺼 둔(`outputs.upToDateWhen { false }`)
> 상태였는데, 명령 문자열만 보는 검사는 그것을 알 수 없다. 미리 말할 수 있는 쪽과 정확한
> 쪽이 다르므로 둘 다 남긴다.
>
