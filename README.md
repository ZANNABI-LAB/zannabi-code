...
> `report.md`에 숫자와 함께 남긴다(실측 사례가 54.9s → 14.8s였다). 명령어 이름을 보는 사전
> 경고는 래퍼 스크립트나 처음 보는 빌드 도구에 뚫리지만, **소요시간은 일이 실제로 일어나지
> 않았다는 직접 신호**다. 다만 데몬이 덥혀져 정직하게 빨라지는 경우도 있어 통과를 무르지는
> 않는다 — 판정은 사람이 한다.
>
