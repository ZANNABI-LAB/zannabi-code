...
| `--escalate` | 정체하면 중단 대신 실행 턴을 **계획 런타임으로 올려** 한 번 더 (기본 꺼짐) |
...
**승격(cascade).** `--escalate`를 주면 정체가 감지됐을 때 중단하는 대신 **실행 턴을 계획
런타임으로 올려** 한 번 더 시도한다. 싸게 시작해 안 되면 강한 모델로 올리는 구조 자체는 흔하지만,
널리 쓰이는 구현은 승격 여부를 **모델의 자기 확신도**로 판정한다 — 작업한 쪽이 자기 작업의 합격
여부를 말하는 구조다. zannabi는 게이트를 직접 돌리므로 **프로세스 밖 종료코드**로 판정한다:
*"연속 N라운드 동안 변경분과 게이트 결과가 똑같다"* 는 이 실행자로는 못 푼다는 관측된 사실이고,
그것이 지금까지 중단 사유였을 뿐이다.

- **계획 런타임으로만 올린다.** 러너는 모델 등급표를 갖지 않는다 — "무엇이 더 강한가"를 도구가
  안다고 주장하면 모델이 바뀔 때마다 틀리고, 틀린 등급으로 조용히 비싼 모델을 부른다. 대신 이미
  **사람이 표명한 판단**(계획 턴에 고른 런타임)을 쓴다. 계획과 실행이 같으면 올릴 곳이 없고,
  그 사실을 **승인 전에** 말한다.
- **한 번만.** 올라갈 데가 하나뿐이라 두 번째는 정의되지 않는다.
- **정체 카운터는 승격 시점에 리셋된다.** 실행자가 바뀌었는데 앞선 실행자의 제자리 라운드까지
  함께 세면 새 실행자가 첫 시도에서 곧바로 끊긴다.
- **기본은 꺼짐이다.** 2026-08-20 측정 11회가 전부 1라운드로 끝나 정체 감지 자체가 아직 실전에서
  발동한 적이 없다. **미검증인 자동 동작이 기본값으로 사람의 지출에 관여해서는 안 된다.**

**정체는 생각보다 잘 안 잡힌다.** 승격을 발동시키려고 세 번 시도했고 세 번 다 실패했는데,
이유가 전부 달랐다 — ① 막힌 에이전트는 가만히 있지 않고 계속 다른 것을 시도한다
② **러너 출력을 대상 저장소 안에 리다이렉트하면**(`zannabi run ... > run.log`) 그 파일이 매 라운드
커져 diff가 항상 달라진다(`.zannabi/`는 diff에서 빼지만 사용자가 만든 로그는 뺄 수 없다 —
**로그는 저장소 밖에 두는 것이 좋다**) ③ 에이전트가 만들었다 지웠다 하는 **A-B-A-B 왕복**은
연속 꼬리가 아니라서 잡히지 않는다. 그래서 `--escalate`를 켜도 당분간 아무 일도 일어나지 않을
가능성이 높다.
