...
export interface AgentAdapter {
  ...
  /**
   * 이 중 자기 확인용으로 **실제로 열 수 있는** 명령은 무엇인가. 선택 구현.
   *
   * 루프는 프롬프트를 쓰기 **전에** 묻는다. 열어 준 목록과 실제 열린 목록이 다른데 그
   * 차이를 아무도 모르면, 프롬프트가 못 쓰는 것을 "쓸 수 있다"고 말한다 — 실측에서
   * 에이전트가 승인된 게이트를 문자 하나까지 그대로 쳤는데 거부당했고, 안내문이
   * *"정확히 베껴라"*였으므로 자기가 무엇을 틀렸는지 찾느라 시도를 한 번 더 썼다.
   *
   * **판정이 어댑터에 있는 이유**: core는 런타임의 권한 규칙을 모른다. claude는 명령
   * 문자열의 접두를 보고 codex는 샌드박스라 이 물음 자체가 없다.
   * 구현하지 않으면 전부 열린다고 본다.
   */
