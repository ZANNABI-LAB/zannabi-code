 ...
 * 조합 프리셋.
 *
 * 한때 난이도를 보고 조합을 **자동으로** 고르는 것이 목표였고, 이 주석도 "판단 신호가 아직
 * 안 쌓였다"고 적고 있었다. **11회 실측 뒤 사전 난이도 추정은 접었다** — 표본이 전부
 * attempts 1이라 라운드 수가 변별 축이 되지 못했고(변별력이 생긴 것은 비용 편차다),
 * 라우팅 문헌도 에이전트 과제에서는 사전 추정을 약하게 본다. 그 자리를 대신한 것이
 * **비용 상한**(`--max-cost`)이다.
 *
 * 그래서 프리셋은 자동 판정의 대용품이 아니라 **근거가 있는 기본값 묶음**으로 남는다.
