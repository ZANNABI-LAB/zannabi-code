...
export function manifest(version: string): Manifest {
  return {
    ...
    capabilities: {
       ...
       * 지울 수도, 고칠 수도 있다. 러너는 손실을 감지해 판정을 `evidence-lost`로 강등하고
       * 저널의 해시 체인이 깨지면 변조 흔적을 알리지만, **둘 다 사후 탐지지 경계가 아니다.**
       *
       * 특히 변조는 **막을 수 없다**: 저널을 통째로 다시 계산하면 체인 검사는 통과한다.
       * 비밀키 없이 위조를 막을 수는 없고, 키를 둘 곳도 없다(증거 옆에 두면 같이 읽힌다).
       * `full`로 신고하면 소비자가 "증거는 항상 있고 늘 참이다"를 전제로 화면을 그리게 된다.
       */
      evidence: 'partial',
