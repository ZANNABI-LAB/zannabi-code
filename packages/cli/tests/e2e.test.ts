...
test('E2E: 플래그가 설정 파일을 이긴다', async () => {
  ...
  const proc = Bun.spawn(
    ['bun', cliPath, 'run', '테스트 작업', '--cwd', project, '--gate', 'user:true', '--yes', '--plan-model', 'from-flag'],
    { env: { ...process.env, ZANNABI_ADAPTER: 'fake' }, stdin: 'ignore', stdout: 'pipe', stderr: 'pipe' },
  )
  await proc.exited
  const out = await new Response(proc.stdout).text()
...
test('E2E: --profile cheap은 실행 턴만 낮추고 계획 턴은 건드리지 않는다', async () => {
  ...
  const proc = Bun.spawn(
    ['bun', cliPath, 'run', '테스트 작업', '--cwd', project, '--gate', 'user:true', '--yes', '--profile', 'cheap'],
    { env: { ...process.env, ZANNABI_ADAPTER: 'fake' }, stdin: 'ignore', stdout: 'pipe', stderr: 'pipe' },
  )
  await proc.exited
  const out = await new Response(proc.stdout).text()
...
test('E2E: 모르는 프리셋 이름은 고를 수 있는 것을 알려준다', async () => {
  ...
  const proc = Bun.spawn(
    ['bun', cliPath, 'run', '테스트 작업', '--cwd', project, '--gate', 'user:true', '--yes', '--profile', 'turbo'],
    { env: { ...process.env, ZANNABI_ADAPTER: 'fake' }, stdin: 'ignore', stdout: 'pipe', stderr: 'pipe' },
  )
  const exitCode = await proc.exited
  const out = (await new Response(proc.stdout).text()) + (await new Response(proc.stderr).text())
...
test('E2E: --profile safe는 런타임을 낮추지 않고 재확인을 켠다', async () => {
  ...
  const proc = Bun.spawn(
    ['bun', cliPath, 'run', '테스트 작업', '--cwd', project, '--gate', 'user:true', '--yes', '--profile', 'safe'],
    { env: { ...process.env, ZANNABI_ADAPTER: 'fake' }, stdin: 'ignore', stdout: 'pipe', stderr: 'pipe' },
  )
  await proc.exited
  const out = await new Response(proc.stdout).text()
